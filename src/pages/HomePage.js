...
import { Box, Button, Image, Input, InputGroup, InputRightElement } from '@chakra-ui/react';
  ...
  return (
    <Box>
      ...
      <Box display='flex' flexWrap='wrap' justifyContent={'space-evenly'}>
        {loading ? <TilesSkeleton /> : currentMovies.map(movie => (
      ...
      <Box display='flex ' justifyContent='center'>
        { pageNumbers[1] ? pageNumbers.map(number => (
          ...
          marginY='5'
          marginX='1'
          backgroundColor={number === currentPage ? 'orange.500' : 'orange.300'}
